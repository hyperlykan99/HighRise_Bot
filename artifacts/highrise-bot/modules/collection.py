...
def _get_mining_total() -> int:
    try:
        ...
        n = conn.execute("SELECT COUNT(*) FROM mining_items").fetchone()[0]
        conn.close()
        return n or 25
    except Exception:
        return 25
...
async def handle_collection(bot: BaseBot, user: User, args: list) -> None:
    ...
    await _w(bot, uid, (
        f"📖 Collection Book\n"
        f"⛏️ Ore:  {mine_d}/{mine_t} ({_pct(mine_d, mine_t)})\n"
        f"🎣 Fish: {fish_d}/{fish_t} ({_pct(fish_d, fish_t)})\n"
        f"Total:  {total_d}/{total_t} ({_pct(total_d, total_t)})\n"
        f"!orebook  !fishbook  !collection rare"
    )[:249])
