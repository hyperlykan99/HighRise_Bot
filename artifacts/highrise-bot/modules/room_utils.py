...
# ---------------------------------------------------------------------------
# Heart cooldown state  (in-memory; resets on restart — intentional)
# uid → timestamp of last heart send
# ---------------------------------------------------------------------------
_heart_cd: dict[str, float] = {}
_HEART_CD_NORMAL = 10.0   # seconds — non-VIP
_HEART_CD_VIP    =  5.0   # seconds — VIP+
_HEART_MAX_VIP   = 20     # max hearts per !hearts for VIP
_HEART_MAX_STAFF = 100    # max hearts per !hearts for staff/owner
...
def is_known_bot_username(username: str | None) -> bool:
    """Return True for canonical bot accounts/aliases so player joins can wake anchors safely."""
    name = str(username or "").strip().lower()
    if not name:
        return False
    known = {u.lower() for u in _CANONICAL_BOT_USERNAMES.values()}
    known.update(_CANONICAL_BOT_USERNAMES.keys())
    for aliases in _BOT_SPAWN_MODE_ALIASES.values():
...
def mark_current_bot_presence_ok() -> None:
    ...
    _guardian_log(mode, username, "presence_ok", cooldown=300.0)
...
def schedule_bot_presence_retry(
    bot: BaseBot,
    reason: str = "not_in_room",
    cooldown: float = 75.0,
    *,
    ...
    async def _run_restore() -> None:
        try:
            ok = await apply_bot_spawn(bot, username, allow_restart=allow_restart)
            if ok is False:
        ...
        while True:
            try:
                ...
                if state == "present":
                    ...
                    _guardian_log(mode, username, "presence_soft_uncertain", f"count={count} reason=self_not_listed", cooldown=300.0)
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                log_cooldown(
...
async def run_delayed_anchor_restores(bot: BaseBot, bot_username: str, delays: tuple[int, ...] = (10, 30, 90)) -> None:
    """Re-anchor a freshly connected bot after Highrise finishes settling it in-room."""
    last_delay = 0
    for delay in delays:
        ...
        try:
            ok = await apply_bot_spawn(bot, bot_username, allow_restart=False)
            if ok:
                mark_current_bot_presence_ok()
            else:
                _guardian_log(mode, username, "anchor_deferred", f"reason=delayed_anchor_restore_failed delay={delay}", cooldown=60.0)
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            _guardian_log(mode, username, "anchor_deferred", f"reason=delayed_anchor_restore_exception delay={delay} error={exc!r}", cooldown=60.0)
            _guardian_log(
                mode,
                username,
                "spawn_restore_failed",
...
async def _send_heart_to_user(bot: BaseBot, target_id: str) -> None:
    """Wrapper around the SDK heart reaction. Raises on failure."""
...
_HEART_DELAY_SLOW   = 0.10   # fallback on rate-limit


async def send_hearts_burst(
    bot: BaseBot,
    sender: User,
    target_user: User,
    amount: int,
    ...
    """THE only heart-sending path — loops amount SDK hearts, announces once.

    Both !heart and !hearts route here for single-target sends.
    Returns count actually sent.
    """
    sid   = sender.id
    delay = (_HEART_DELAY_STAFF if is_staff
             else _HEART_DELAY_VIP if is_vip
             else 0.15)

    print(f"[HEART_BURST_START] target={target_user.id} amount={amount}")
        ...
        try:
            ...
            if current_delay < _HEART_DELAY_SLOW:
                ...
                try:
                    ...
                    print(f"[HEART_SEND_FAIL] sid={sid} tid={target_user.id} {exc2!r}")
                    break
            else:
                print(f"[HEART_SEND_FAIL] sid={sid} tid={target_user.id} {exc!r}")
                break
        if amount > 1:
            await asyncio.sleep(current_delay)

    print(f"[HEART_BURST_DONE] target={target_user.id} sent={sent}")

    if not sent:
        await _w(bot, sid, "Heart API unavailable — no hearts sent.")
        return 0

    db.give_hearts_bulk(sender.username, target_user.username, sent)

    if not is_staff:
        _heart_cd[sid] = time.monotonic()

    h = "heart" if sent == 1 else "hearts"
    await bot.highrise.chat(
        f"💖 @{sender.username} sent {sent} {h} to @{target_user.username}."[:249])
    return sent


def _heart_parse_tokens(tokens: list[str]) -> tuple[bool, bool, str | None, int | None]:
    """Parse token list (args[1:]) for heart commands.

    Returns (is_all, include_me, target_name, amount).
    Any token order accepted: @user N | N @user | all N [include me]
    amount is None when not present.
    """
    joined = " ".join(tokens).lower()
    include_me = "include me" in joined
    is_all     = False
    target: str | None = None
    amount: int | None = None

    skip = {"heart", "hearts", "include", "me"}
    for tok in tokens:
    ...
    return is_all, include_me, target, amount


async def _burst_all(bot: BaseBot, user: User,
                     amount: int, include_me: bool) -> None:
    """Staff-only: burst N hearts to every non-bot player in the room."""
    uid = user.id
    ...
    player_count = 0
    for t in targets:
        print(f"[HEART_BURST_START] target={t.id} amount={amount}")
        c_delay = _HEART_DELAY_STAFF
        sent = 0
        for _ in range(amount):
            try:
                await _send_heart_to_user(bot, t.id)
                sent += 1
            except Exception:
                if c_delay < _HEART_DELAY_SLOW:
                    c_delay = _HEART_DELAY_SLOW
                    print(f"[HEART_RATE_LIMIT_SLOWDOWN] delay={_HEART_DELAY_SLOW}")
            ...
            if amount > 1:
                await asyncio.sleep(c_delay)
        print(f"[HEART_BURST_DONE] target={t.id} sent={sent}")
        if sent:
...
async def handle_heart(bot: BaseBot, user: User, args: list[str]) -> None:
    ...
    Everyone:  !heart @user       → 1 heart (cooldown applies)
    VIP+:      !heart @user N     → up to 20, 5s cooldown
    Staff:     !heart @user N     → up to 100, no cooldown
    Staff:     !heart all [N]     → burst N hearts to every player
    """
    uid      = user.id
    ...
    is_all, include_me, target_name, parsed_n = _heart_parse_tokens(args[1:])
    ...
    print(f"[HEART_PARSE] cmd=heart target={target_name!r} amount={amount}")

    if amount > 1 and not is_staff and not is_vip:
        await _w(bot, uid, "VIP+ required to send multiple hearts.")
        return
    ...
    if is_all:
        if not is_staff:
            ...
            return
        await _burst_all(bot, user, amount, include_me)
        return
    ...
    await send_hearts_burst(bot, user, target_user, amount, is_staff, is_vip)


async def handle_hearts(bot: BaseBot, user: User, args: list[str]) -> None:
    """!hearts @user <N> [include me]  — VIP+/staff only.
    ...
    print(f"[HEART_PARSE] cmd=hearts target={target_name!r} amount={amount}")
...
async def teleport_bot_to_saved_spawn(
    ...
    if bot_uid:
        try:
            ...
            if _exc_contains(exc, "server error"):
                teleport_failed_reason = "teleport_server_error"
            elif _exc_contains(exc, "not in room") or _exc_contains(exc, "user not in room"):
                teleport_failed_reason = "teleport_not_in_room"
            if is_host and teleport_failed_reason == "teleport_server_error":
                print(
    ...
    if fallback_walk:
        ...
        try:
            ...
            if _exc_contains(exc, "not in room") or _exc_contains(exc, "user not in room"):
                reason = "walk_not_in_room"
                if teleport_failed_reason:
                    reason = f"{teleport_failed_reason}_walk_not_in_room"
                if is_host:
                    print(
                        f"[SPAWN_RESTORE] host_spawn_restore_not_in_room_retry "
                        f"bot={_username!r} reason=walk_not_in_room"
                    )
                return (False, pos, row, reason, saved_key) if return_details else False
    else:
        print("[BOT SPAWN] fallback_walk=false")

    reason = teleport_failed_reason or "teleport_failed"
    return (False, pos, row, reason, saved_key) if return_details else False


async def apply_bot_spawn(bot: BaseBot, bot_username: str, *, allow_restart: bool = False) -> bool:
    ...
    try:
        for attempt, delay in enumerate(delays, 1):
            ...
            if hard_restore_failure:
                ...
                if allow_restart:
                    ...
                    _guardian_log(mode, username, "anchor_deferred", f"reason={reason}", cooldown=60.0)
            if is_host:
                print(
        ...
        if allow_restart:
            ...
            _guardian_log(
                ...
                username,
                "anchor_deferred",
                f"reason=spawn_restore_gave_up hard_failures={hard_failure_count} actual={_format_pos(actual)}",
                cooldown=60.0,
            )
        return False
    finally:
