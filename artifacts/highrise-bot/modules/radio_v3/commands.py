...
from modules.radio_v3 import payments
from modules.radio_v3 import player
from modules.radio_v3 import queue
from modules.radio_v3 import renderer
from modules.radio_v3 import settings
from modules.radio_v3 import sources_local
from modules.radio_v3 import sources_youtube
...
async def _accept_request(
    ...
    *,
    ...
    if source_type == "youtube":
        asyncio.create_task(sources_youtube.prepare(bot, request_id, source_ref), name=f"radio_v3_yt_{request_id}")
    else:
        asyncio.create_task(sources_local.prepare(bot, request_id, favorite or {}), name=f"radio_v3_local_{request_id}")
    return request_id
...
async def handle_nowplaying(bot, user, _args: list[str]) -> None:
    ...
    if not np:
        ...
        return
    song, _media, elapsed, duration = player._extract(np)
    row = player.match_nowplaying(np)
    if row:
...
async def handle_cancel(bot, user, args: list[str]) -> None:
    ...
    if not jobs:
        current = player.current_request()
    ...
    row = jobs[idx]
    rid = int(row["id"])
    queue.mark_status(rid, "cancelled", error="cancelled_by_user")
    refunded = payments.refund_if_needed(row)
    if refunded:
        queue.update_request(rid, song_play_refunded=1)
    loop = asyncio.get_running_loop()
    await loop.run_in_executor(None, cleanup.cleanup_request_media, queue.get_request(rid), "cancelled_by_user")
...
async def handle_remove(bot, user, args: list[str]) -> None:
    ...
    if len(args) < 2 or not args[1].isdigit():
        await _w(bot, user.id, "📋 Pending (use !remove <#>)" if jobs else "📋 No pending requests to remove.")
        return
    idx = int(args[1]) - 1
    if idx < 0 or idx >= len(jobs):
    ...
    if payments.refund_if_needed(row):
        queue.update_request(rid, song_play_refunded=1)
    loop = asyncio.get_running_loop()
    await loop.run_in_executor(None, cleanup.cleanup_request_media, queue.get_request(rid), "removed_by_staff")
    await _w(bot, user.id, f"✅ Removed: {(row.get('title') or 'request')[:40]}")


async def handle_skip(bot, user, _args: list[str]) -> None:
    player.start(bot)
    ok, msg = await player.skip_current_request(bot, user.id, staff=payments.is_staff(user.username))
    await _w(bot, user.id, msg)


async def handle_voteskip(bot, user, _args: list[str]) -> None:
    player.start(bot)
    _ok, msg = await player.vote_skip(bot, user.id)
    await _w(bot, user.id, msg)


async def handle_myrequests(bot, user, _args: list[str]) -> None:
    await _send_pages(bot, user.id, renderer.request_history(queue.recent_for_user(user.id, 6)))
...
async def handle_requesthistory(bot, user, _args: list[str]) -> None:
    if not payments.is_staff(user.username):
        ...
        return
    await _send_pages(bot, user.id, renderer.request_history(queue.recent_room(10), title="📜 Radio History"))
...
async def handle_playfav(bot, user, args: list[str]) -> None:
    ...
    fav = rows[pos - 1]
    if fav.get("url"):
        await _accept_request(bot, user, title=fav.get("title") or "Preparing…", artist=fav.get("artist") or "", source_type="youtube", source_ref=fav.get("url") or "")
    elif fav.get("azura_file_id"):
        await _accept_request(bot, user, title=fav.get("title") or "Local favorite", artist=fav.get("artist") or "", source_type="local_favorite", source_ref=str(fav.get("id") or ""), favorite=fav)
    else:
    ...
    for row in queue.active_for_user(user_id):
        ...
        if row.get("status") == "playing":
            if settings.refund_if_leaves() and payments.refund_if_needed(row):
                queue.update_request(rid, song_play_refunded=1)
            await player.skip_current_request(bot, user_id, staff=True)
            ...
            if settings.refund_if_leaves() and payments.refund_if_needed(row):
                queue.update_request(rid, song_play_refunded=1)
            loop = asyncio.get_running_loop()
            await loop.run_in_executor(None, cleanup.cleanup_request_media, queue.get_request(rid), "requester_left")
