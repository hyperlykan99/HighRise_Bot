"""Radio V3 YouTube source acquisition."""
...
async def prepare(bot, request_id: int, url: str) -> None:
    row = queue.get_request(request_id)
    if not row:
        return
    tmpdir = tempfile.mkdtemp(prefix="radio_v3_yt_")
    staged = ""
    try:
        ...
        safe_name = f"request_v3_{request_id}_{video_id or os.path.basename(mp3_path)[:16]}.mp3"
        ...
        result = await loop.run_in_executor(None, v3_azura.finalize_uploaded_request, request_id, safe_name, title)
        if not result.get("ok"):
            raise RuntimeError(result.get("error") or "media_ready_failed")
        queue.mark_status(
            request_id,
            "ready",
