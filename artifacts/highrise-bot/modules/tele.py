...
# ---------------------------------------------------------------------------
# !rolemembers <role> — list players who have a given role tag
# ---------------------------------------------------------------------------

async def handle_rolemembers(bot: BaseBot, user: User, args: list[str]) -> None:
    """!rolemembers <role> — show all members of a role tag."""
    if len(args) < 2:
        await _w(bot, user.id,
                 "Usage: !rolemembers <role>\n"
                 "e.g. !rolemembers vip\n"
                 "Roles: vip, mod, staff, manager, admin, owner, supporter, member")
        return
    role = args[1].lower()
    try:
        conn = db.get_connection()
        # Try room_tag_members table first (if role exists as a tag)
        rows = conn.execute(
            "SELECT username FROM room_tag_members WHERE tag=? ORDER BY username",
            (role,),
        ).fetchall()
        conn.close()
    except Exception:
    ...
    # Fallback: check staff roles from users table
    if not rows:
        try:
            from modules.permissions import (
                get_owners, get_admins, get_managers, get_moderators,
            )
            role_getters: dict[str, object] = {
                "owner": get_owners,
                "admin": get_admins,
                "manager": get_managers,
                "mod": get_moderators,
                "moderator": get_moderators,
            }
            getter = role_getters.get(role)
            if getter:
                members = list(getter())  # type: ignore[call-arg]
                if members:
                    lines = [f"👥 Role: {role.title()} ({len(members)})"]
                    lines += [f"  @{m}" for m in sorted(members)[:10]]
                    if len(members) > 10:
        ...
        if role in ("vip", "vips"):
            try:
                ...
                if vip_list:
                    lines = [f"👥 VIP Members ({len(vip_list)})"]
                    lines += [f"  @{v}" for v in sorted(vip_list)[:10]]
                    if len(vip_list) > 10:
                        lines.append(f"  ...+{len(vip_list)-10} more")
                    await _w(bot, user.id, "\n".join(lines)[:249])
                    return
            except Exception:
                pass
        await _w(bot, user.id,
                 f"No members found for role '{role}'.\n"
                 "Check !roles for available role spawns.")
        return

    members = [r["username"] for r in rows]
    lines = [f"👥 Role Members: {role.title()} ({len(members)})"]
    lines += [f"  @{m}" for m in sorted(members)[:10]]
    if len(members) > 10:
        lines.append(f"  ...+{len(members) - 10} more")
    await _w(bot, user.id, "\n".join(lines)[:249])
