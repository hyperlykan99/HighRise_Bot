...
DJ_COMMANDS: frozenset[str] = frozenset({
    ...
    "radioachievements", "radioach", "radioawards",
    "tipdj",
    "djvibes",
    "vibe", "vibes", "vibescan",
    "djreport",
...
class HangoutBot(BaseBot):
    ...
    async def _on_chat_impl(self, user: User, message: str) -> None:
        ...
        if cmd in {"balance", "bal", "b", "coins", "coin", "money"}:
            ...
            first_arg = args[1].lower().strip() if len(args) > 1 else ""
            if not first_arg:
                await handle_shop_badges(self, user, args)
            else:
            ...
            first_arg = args[1].lower().strip() if len(args) > 1 else ""
            if not first_arg:
                await handle_badgeshop_categories(self, user)
            else:
                await handle_badges_cmd_router(self, user, args)
        ...
        # ── Emote commands — all bots ─────────────────────────────────────────
            ...
            await handle_unresolvedplayeremotes(self, user, args)
        elif cmd == "emoteid":
            await handle_emoteid(self, user, args)
        elif cmd == "playeremoteid":
        ...
        # ── New emote_extras commands ────────────────────────────────────────
            ...
            elif cmd == "addworkingemote":
                await handle_addworkingemote(self, user, args)
            elif cmd == "removeworkingemote":
                await handle_removeworkingemote(self, user, args)
            elif cmd == "workingcount":
