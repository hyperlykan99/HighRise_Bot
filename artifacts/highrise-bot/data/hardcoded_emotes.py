...
# ---------------------------------------------------------------------------
# TIMED_EMOTES — full confirmed timing table (220 entries)
# Source: verified SDK measurements.  Used by both bot and player loops.
# ---------------------------------------------------------------------------
TIMED_EMOTES: list[dict] = [
    ...
    {"value": "sit-relaxed",            "text": "Relaxed Sit",        "time": 29.889858},
    ...
    {"value": "emote-kawaiigogo",       "text": "KawaiiGoGo",         "time": 10.0},
    ...
    {"value": "dance-employee",         "text": "Push it",            "time": 8.0},
]

# Fast lookup: emote_id → duration in seconds.  Fallback is 5.0s.
TIMED_EMOTES_BY_ID: dict[str, float] = {
    item["value"]: float(item["time"])
    for item in TIMED_EMOTES
}
